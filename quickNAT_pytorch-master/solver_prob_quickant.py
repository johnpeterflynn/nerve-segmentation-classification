...
class SolverProbQuickNat(object):
        ...
        for epoch in range(self.start_epoch, self.num_epochs + 1):
            ...
            for phase in ['train', 'val']:
                ...
                for i_batch, sample_batched in enumerate(dataloaders[phase]):
                    ...
                    if model.is_cuda:
                        X, y = X.cuda(self.device, non_blocking=True), \
                            y.cuda(self.device, non_blocking=True)

                    output = model(X, torch.unsqueeze(y, 1))

                    elbo_loss = self.elbo(y)
                ...
                with torch.no_grad():
                    ...
                    index = np.random.choice(
                        len(dataloaders[phase].dataset.X), 3, replace=False)
    ...
    def elbo(self, ground_truth_seg, weighting_coff=None, analytic_kl=True, reconstruct_posterior_mean=False):
        """
        ...
        reconstruction_loss = self.loss_func(
            input=self.reconstruction,
            target=ground_truth_seg,
            weight=weighting_coff)

        self.reconstruction_loss = torch.sum(reconstruction_loss)
        self.mean_reconstruction_loss = torch.mean(reconstruction_loss)

        return -(self.reconstruction_loss + self.model.beta * self.kl)
