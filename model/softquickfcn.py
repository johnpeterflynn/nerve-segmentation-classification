...
class SoftQuickFCN(BaseModel):
    ...
    def __init__(self, params):
        ...
        self.cross1ss = torch.nn.Parameter(data=torch.ones(1, params['num_filters'], 1, 1), requires_grad=True)
        self.cross1sc = torch.nn.Parameter(data=torch.zeros(1, params['num_filters'], 1, 1), requires_grad=True)
        self.cross1cc = torch.nn.Parameter(data=torch.ones(1, params['num_filters'], 1, 1), requires_grad=True)
        self.cross1cs = torch.nn.Parameter(data=torch.zeros(1, params['num_filters'], 1, 1), requires_grad=True)
        ...
        self.cross2ss = torch.nn.Parameter(data=torch.ones(1, params['num_filters'], 1, 1), requires_grad=True)
        self.cross2sc = torch.nn.Parameter(data=torch.zeros(1, params['num_filters'], 1, 1), requires_grad=True)
        self.cross2cc = torch.nn.Parameter(data=torch.ones(1, params['num_filters'], 1, 1), requires_grad=True)
        self.cross2cs = torch.nn.Parameter(data=torch.zeros(1, params['num_filters'], 1, 1), requires_grad=True)
        ...
        self.cross3ss = torch.nn.Parameter(data=torch.ones(1, params['num_filters'], 1, 1), requires_grad=True)
        self.cross3sc = torch.nn.Parameter(data=torch.zeros(1, params['num_filters'], 1, 1), requires_grad=True)
        self.cross3cc = torch.nn.Parameter(data=torch.ones(1, params['num_filters'], 1, 1), requires_grad=True)
        self.cross3cs = torch.nn.Parameter(data=torch.zeros(1, params['num_filters'], 1, 1), requires_grad=True)
        ...
        self.crossbss = torch.nn.Parameter(data=torch.ones(1, params['num_filters'], 1, 1), requires_grad=True)
        self.crossbsc = torch.nn.Parameter(data=torch.zeros(1, params['num_filters'], 1, 1), requires_grad=True)
        self.crossbcc = torch.nn.Parameter(data=torch.ones(1, params['num_filters'], 1, 1), requires_grad=True)
        self.crossbcs = torch.nn.Parameter(data=torch.zeros(1, params['num_filters'], 1, 1), requires_grad=True)
